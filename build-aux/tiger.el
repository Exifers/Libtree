;;; tiger.el --- Mode for editing programs written in EPITA's
;;; object-aware version of the Tiger language, derived from Andrew
;;; Appel's.
...
;;
;;
;; Since 1999, 3rd grade students at EPITA[2] have a compiler
;; construction assignment called the Tiger Project[3].  This project
;; is based on Appel's books.  In 2007, the Tiger Project temporarily
;; became the Leopard Project: the language was extended with
;; object-oriented programming features (based on Appel's proposal),
;; and renamed to Leopard[4].  The language was later renamed back to
;; Tiger, and the initial (object-less) Tiger language was dubbed
;; Panther.  This Emacs mode was derived from the one written for that
;; latter language, and augmented to support new keywords.
;;
;;                                                           -- Roland
;;
;; 2. <http://www.epita.fr/>
;; 3. <http://www.lrde.epita.fr/~tiger/assignments.split>
;; 4. <http://www.lrde.epita.fr/~tiger/tiger.html>
;;
;;
;; FIXME: BTW, if there is an easy way to use a kind of inheritance
;; between Emacs major modes, I (Roland) will be glad to use it to
;; make this Tiger mode inherit from Panther's and factor common
;; parts.
...
(defconst tiger-font-lock-keywords
  `(
    ;; Tiger keywords:
    ("\\<\\(array\\|break\\|class\\|do\\|e\\(?:lse\\|nd\\|xtends\\)\\|f\\(?:or\\|unction\\)\\|i\\(?:f\\|mport\\|n\\)\\|let\\|method\\|n\\(?:ew\\|il\\)\\|of\\|primitive\\|t\\(?:hen\\|o\\|ype\\)\\|var\\|while\\)\\>"
